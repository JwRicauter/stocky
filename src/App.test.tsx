...
describe("Symbols Component", () => {
  ...
  it("should render symbols when api respond", async () => {
    ...
    return api.getSymbols(signal).then(onResponse).catch(onError).finally(() => {
...
describe("Testing stock candles api", () => {
  ...
  it("should render stock candels ok with no symbol", async () => {
    ...
    return api.getCandles(signal, [], '1681164000', '1682460000').then(onResponse).catch(onError).finally(() => {
      expect(onResponse).toHaveBeenCalled();
      expect(onError).not.toHaveBeenCalled();
  ...
  it("should render stock candels ok with one symbol", async () => {
    ...
    return api.getCandles(signal, ['AAPL'], '1681164000', '1682460000').then(onResponse).catch(onError).finally(() => {
      expect(onResponse).toHaveBeenCalled();
      expect(onError).not.toHaveBeenCalled();
  ...
  it("should show time chart", async () => {
    ...
    let chartComponent = render(<Charts 
      dateRange={[oneWeekAgo, today]}
      priceType={'High Prices'}
      selectedSymbols={['AAPL']}
    />)
    
    await waitFor(() => expect(chartComponent.getByTestId('chart')).toBeInTheDocument());
